...
RSpec.describe Api::V1::LinesController, type: :controller do
  ...
  describe "#index" do
    ...
    describe "with no authenticated users" do
      it "should not list line" do
        get :index, format: :json
        expect(assigns[:lines]).to eq(nil)
      end
    ...
    describe "with authenticated user" do
      before :each do
        @user = create(:user)
        authenticate(@user.auth_token, controller)
      end
  ...
  describe "#create" do
    ...
    describe "with authenticated user" do
      ...
      describe "with valid parameters" do
        ...
        it "should create a new line for the current_user" do
          user2 = create(:user)
          
          expect do
            expect do
              post :create, params: @line_attributes, format: :json
            end.to change(@user.reload.lines, :count).by(1)
          end.not_to change(user2.reload.lines, :count)

          subject.instance_variable_set(:@current_user, user2)
          expect do
            expect do
              post :create, params: @line_attributes, format: :json
            end.to change(user2.reload.lines, :count).by(1)
          end.not_to change(@user.reload.lines, :count)
        end
